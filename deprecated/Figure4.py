...
print(f"saving combined X_mini and Target_mini to {Path}Ns{N}_SeqN{T}_2Batch.pth.tar")
torch.save(
...
plt.subplot(2, 3, 1)
CA1_MI = np.array(
    [MI_linear(CA1_rep[0][:, i], norm=False) for i in range(CA1_rep[0].shape[1])]
)
...
plt.subplot(2, 3, 3)
CA1_MI = np.array(
    [MI_linear(CA1_rep[1][:, i], norm=False) for i in range(CA1_rep[1].shape[1])]
)
PF_idx = np.argwhere(CA1_MI > CA1_thres)
...
for i in range(len(epoch_list) - 1):
    MI_CA1[i, :] = np.array(
        [
            MI_linear(CA1_rep[i + 1, :, neuron_idx], norm=False)
            for neuron_idx in range(CA1_rep.shape[2])
        ]
    )
    MI_CA3[i, :] = np.array(
        [
            MI_linear(CA3_rep[i + 1, :, neuron_idx], norm=False)
            for neuron_idx in range(HN)
        ]
    )
...
torch.save(
    {"onset_idx_CA1": onset_idx_CA1, "onset_idx_CA3": onset_idx_CA3},
...
for noise_level2 in noise_level2_list:
    ...
    corr_CA3 = np.array(
        [
            np.corrcoef(
                env1["CA3_rep"][-1, :, neuron_idx], env2["CA3_rep"][-1, :, neuron_idx]
            )[0, 1]
            for neuron_idx in range(CA3_rep.shape[2])
        ]
    )
    corr_CA1 = np.array(
        [
            np.corrcoef(
                env1["CA1_rep"][-1, :, neuron_idx], env2["CA1_rep"][-1, :, neuron_idx]
            )[0, 1]
            for neuron_idx in range(CA1_rep.shape[2])
        ]
    )
    torch.save(
        {"corr_CA1": corr_CA1, "corr_CA3": corr_CA3},
        Path
        + "env{}_env{}_Field2_corr.pth.tar".format(
            int(noise_level1 * 100), int(noise_level2 * 100)
        ),
    )
...
for noise_level2 in noise_level2_list:
    tmp = torch.load(
        Path + "env5_env{}_Field2_corr.pth.tar".format(int(100 * noise_level2))
    )
...
plt.plot(
    noise_level2_list, CA1_mean, marker="o", linestyle="--", label="CA1", color="red"
)
plt.errorbar(
    noise_level2_list, CA1_mean, yerr=np.array(CA1_sem) / 2, fmt="o", color="red"
)
plt.plot(
    noise_level2_list, CA3_mean, marker="o", linestyle="--", label="CA3", color="blue"
)
plt.errorbar(
    noise_level2_list, CA3_mean, yerr=np.array(CA3_sem) / 2, fmt="o", color="blue"
)
plt.legend()
plt.ylabel("Correlation (mean $\pm$ sem)")
